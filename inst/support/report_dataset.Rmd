...
for (trait in seq_along(1:length(traits_numeric)) {

  data_trait_study <- data_study$traits %>%
    filter(trait_name == traits_numeric[[trait]], !value_type %in% c("bin", "range")) %>%
  ...
  transforms <- metadata$traits %>% austraits::convert_list_to_df2() %>% filter(trait_name == traits_numeric[[trait]])

  elements <- definitions[[traits_numeric[[trait]]]]

  units <- elements$units

  c("", "",
    sprintf("### %s", traits_numeric[[trait]]),
    "",
    sprintf("We aligned the contributed variable called `%s` in the data you supplied with the trait `%s` in the AusTraits database, with value_types `%s`.\n\nThe original variable was supplied with units `%s`; these were converted to our standard `%s`.\n\nThe data has the following properties:",
            transforms$var_in %>% paste0(collapse = ", "), traits_numeric[[trait]],
            transforms$value_type %>% paste0(collapse = ", "),
            transforms$unit_in[1], elements$units),
    "",
    sprintf("- **standardised name**: %s", traits_numeric[[trait]]),
...
if (nrow(filter(data_study$excluded_data, error == "Value out of allowable range" & trait_name == traits_numeric[[trait]])) > 0) {
  data_study$excluded_data %>% filter(error == "Value out of allowable range" & trait_name == traits_numeric[[trait]]) %>%
  ...
  austraits::plot_trait_distribution_beeswarm(austraits_no_bins, traits_numeric[[trait]], "dataset_id", highlight = dataset_id, hide_ids = TRUE)
