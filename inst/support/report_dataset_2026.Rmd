...
source(system.file("support", "plot_trait_values.R", package = "traits.build"))
...
`AusTraits` is an open-source compilation of trait data for Australian plant taxa. It includes data for more than 500 morphological, ecological and physiological variables that: 1. Tend to vary among taxa; and 2. Have been collected across a number of taxa. Many people collect trait data, and as such, data is spread across many sources, under a variety of formats and terms.

AusTraits combines all of these data fragments into a harmonised, error checked, publicly available compilation, built to make trait data reusable.

AusTraits included more than 400 unique `datasets`, each denoted by a distinct `dataset_id`. These include individual research datasets generally linked to a scientific publication, morphological and ecological trait data extracted from flora descriptions and the taxonomic literature, informal compilations from the literature submitted by researchers, and unpublished datasets. Our data processing pipeline combines all of the studies in a transparent and reproducible way using [traits.build](http://traitecoevo.github.io/traits.build/). We seek to retain all metadata associated with the individual trait values, including measurement methods, location and location properties, and any additional contextual metadata submitted by the data contributor. 
...
AusTraits exists and continues to expand because of the contributions of researchers across Australia. Thank you for being part of an endeavor to make trait data for Australia's flora readily accessible for ecological research, biodiversity infrastructure and conservation decision-making.
...
Each study is assigned a unique `dataset_id`, with information on that location in its own folder, organised into two files:
...
1. Queries in orange scattered throughout the report. Consider these prompts to faciliate an efficient review of the dataset. 

2. Queries in blue that appear at the end of the report. These are explicit questions pertaining to your dataset that the dataset curator had when adding your study. 
...
3. Sending back your responses and any edited files.
...
This includes the first records in AusTraits for many trait by taxon combinations - thanks for helping to gap-fill our knowledge about Australian plants.
...
```{r categorical, results='asis', echo=FALSE, eval=TRUE, fig.height=8, fig.width=10}
writeLines(
...
for (trait in traits_categorical) {
  ...
  print(plot_categorical_trait_distribution(plot_data_study, trait, family_count = 50))

  writeLines("\n**All of AusTraits:**\n")
  print(plot_categorical_trait_distribution(plot_data_all, trait, family_count = 50))
