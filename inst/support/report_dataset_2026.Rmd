...
Many people collect trait data, and as such, data is spread across many sources, under a variety of formats and terms. AusTraits combines all of these data fragments into a harmonised, error checked, publicly available compilation, built to make trait data reusable. 

AusTraits now includes more than 400 unique `datasets`, each denoted by a distinct `dataset_id`. These include individual research datasets generally linked to a scientific publications, morphological and ecological trait data extracted from flora descriptions and the taxonomic literature, informal compilations from the literature submitted by researchers, and unpublished datasets. Our data processing pipeline combines all of the studies in a transparent and reproducible way using a AusTraits-family R package, [traits.build](http://traitecoevo.github.io/traits.build/). We seek to retain all metadata associated with the individual trait values, including measurement methods, location and location properties, and any additional contextual metadata submitted by the data contributor. 
...
AusTraits exists and continues to expand because of the contributions of researchers across Australia. Thank you for being part of an endeavor to make trait data for Australia's flora readily accessible for ecological and evolutionary research, biodiversity infrastructure and conservation decision-making.
...
1. *Queries in orange scattered throughout the report.* These are automatically generated for all reports and do not indicate we have a concern about your dataset submission; consider them prompts to draw your attention to each section of the report. 

2. *Queries in blue that appear at the start of the report.* These are explicit questions pertaining to your dataset that the dataset curator had when adding your study. 
...
- the fields `taxon_name`, `trait_name`, `value` (for long-format dataset), & `location_name` indicate the column from `data.csv` we have mapped in for this information
...
- custom_R_code is just that; code specifically written to manipulate `data.csv` as it is read in
...
```{r, results='asis', echo=FALSE}
sprintf("The `identifiers` table is defined as: *%s*", schema$metadata$elements$identifiers$description) %>%
  writeLines()
...
```{r, results='asis', echo=FALSE}
if (nrow(data_study$traits) > 0 && isTRUE(all(data_study$traits$entity_type == "species"))) {
...
```{r, results='asis', echo=FALSE}
# The schema's `category` field description opens with its own lead-in
# sentence ("The category of contextual data.") before listing the five
# category options -- we want our own lead-in instead, so strip the
# schema's and splice ours on, but keep pulling the actual (live) list of
# categories/definitions from the schema so it stays in sync automatically.
category_options_text <- schema$metadata$elements$contexts$elements$category %>%
  stringr::str_remove("^[^.]*\\.\\s*") %>%
  stringr::str_replace("^Options", "options")

sprintf(
  "Contexts are %s. There are five context categories: %s",
  schema$metadata$elements$contexts$description %>% stringr::str_remove("\\.\\s*$"),
...
if (length(context_properties) > 0) {
    ...
    for (i in seq_along(context_properties)) {
      ...
      writeLines(paste0("**", context_properties[[i]]$context_property[1], "** has been ",
      ...
      context_table <- context_properties[[i]] %>% dplyr::select(dplyr::any_of(c("find", "value", "description")))
  ...
  if (description_removed) {
    new_question("(section `contexts`) For some contexts, we do not map in descriptions of individual context values; for instance for context values that are numbers, names of seasons, or species names. Let us know if you feel like omitting descriptions for any of your context values leads to ambiguity in their meaning.")
  }
  } else {
...
We have records on `r data_study$traits$taxon_name %>% unique() %>% length()` taxa from your study. The full list of names for taxa recorded in your study are listed at the end of the report.
...
if (length(traits_numeric) > 0) {
  ...
  # standard_deviation/unknown (not meaningful plotted as a raw value)
...
if (nrow(filter(data_study$excluded_data, error == "Value out of allowable range" & trait_name == trait)) > 0) {
  ...
  if (nrow(outliers) > 0) {
    writeLines(
      sprintf("POSSIBLE OUTLIERS: %s of points in your dataset lie in either the bottom or top 2.5 percent of data\n",
      nrow(outliers))
    )
    percent_outliers <- nrow(outliers) / nrow(data_trait_study) * 100
    if (percent_outliers > 10) {
      new_question(paste0("(section `traits`) More than 10% of your data points for the trait `", trait, "` are in the either the bottom or top 2.5 percent of the data for this trait. Does this seem reasonable, given what you know about the biology of these taxa and overall distribution of values in AusTraits?"))
    } else {
      new_question(
...
# actually made, so it's what we gate this whole block on.
