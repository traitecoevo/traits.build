...
On the traits.build-template repository, there is a folder titled `test_dataset_2` within the data folder. 
...
Notice that the second trait in this dataset is documented `specific leaf area`, the inverse of the trait concept `leaf mass per area`. The unit conversions algorithm inverts data read in as specific leaf area, converting it to leaf mass per area.
...
2.  Map in a table of substitutions using `metadata_add_substitutions_table`:
...
The ability to pivot is important for 2 reasons:\ 
