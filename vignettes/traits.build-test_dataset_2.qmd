---
title: 'Instructions for adding dataset: test_dataset_2'
...
In the traits.build-template repository, there is a folder titled `test_dataset_2` within the data folder. 

-   Ensure that this folder exists on your computer. 

-   The file `data.csv` exists within the `test_dataset_2` folder. 

-   There is a folder `raw` nested within the `test_dataset_2` folder, that contains two files, `locations.csv` and `notes.txt`. 
...
For this dataset, location data is provided as a standalone spreadsheet, located in the raw data folder: `test_dataset_2\raw\locations.csv`
...
The file `data/test_dataset_2/raw/test_dataset_2_notes.txt` indicates the main data_contributor for this study.\

Fill in the remaining contributor information as described in the [`test_dataset_1`tutorial](https://github.com/traitecoevo/traits.build/tree/develop/vignettes).\

#### **Dataset fields**

The file `data/test_dataset_2/raw/test_dataset_2_notes.txt` indicates how to fill in the `unknown` dataset fields for this study.

#### **Trait details**

The file `data/test_dataset_2/raw/test_dataset_2_notes.txt` indicates how to fill in the `unknown` trait fields for this study, but see below as well.
...
traits.build_database$excluded_data %>%
  filter(dataset_id == "test_dataset_2") %>%
  View()
```
...
metadata_add_substitutions_list("test_dataset_2", table)
```

All required substitutions have been added to the `metadata.yml` file.

If you were to rerun `dataset_test("test_dataset_2")` the error referring to `Unsupported trait values` would now have vanished.
...
-   The file `scripts/custom.R` that you sourced at the beginning of this tutorial contains customised functions commonly used in custom_R_code. 
...
```{r eval=FALSE}
metadata_check_custom_R_code("test_dataset_2") %>% View()
```
...
The solution is to modify the existing custom_R_code, adding one of the customised functions from the file `scripts/custom.R`, `replace_duplicates_with_NA`: 
...
1 test_dataset_2 Actinotus minor 010            leaf_area          18.8             mm2   population  02         
2 test_dataset_2 Actinotus minor 010            leaf_dry_mass      7                mg    population  02         
3 test_dataset_2 Actinotus minor 010            leaf_mass_per_area 344.827586206897 g/m2  population  02         
4 test_dataset_2 Actinotus minor 011            leaf_area          75.9             mm2   population  03         
5 test_dataset_2 Actinotus minor 011            leaf_dry_mass      7                mg    population  03         
6 test_dataset_2 Actinotus minor 011            leaf_mass_per_area 89.2857142857143 g/m2  population  03         
7 test_dataset_2 Actinotus minor 012            plant_growth_form  herb             NA    species     NA      
```
