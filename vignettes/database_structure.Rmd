...
The traits table includes 12 identifiers, `dataset_id`, `observation_id`, `taxon_name`, `population_id`, `individual_id`, `temporal_id`, `source_id`, `location_id`, `entity_context_id`, `plot_id`, `treatment_id`, and `method_context_id`.
...
labelled as either `numeric` or `categorical`. An example of each type is as follows. For the full list, see the [Trait definitions vignette](http://traitecoevo.github.io/traits.build/articles/trait_definitions.html).
