...
tmp <- 
  unique(database_columns$row_n) %>%
  sort() %>%
  as.data.frame() %>%
  rename(table_seq = 1) %>%
  mutate(row_seq = dplyr::row_number())

for (i in seq_along(1:length(tmp))) {
  anchor <- database_tables$anchor[match(names(database_columns_sub_0)[[i]], database_tables$row_n)] #English label to print
  schema_label <- database_tables$alt_label[match(names(database_columns_sub_0)[[i]] %>% as.numeric(), database_tables$row_n)] #schema label to extract columns from table
  
  writeLines(sprintf('\n\n <span id="%s"> </span>\n\n### %s ', anchor, schema_label))
  
  column_seq_vec <- 
    y$austraits$elements[[schema_label]]$elements %>%
    names() %>%
    as.vector()
  ...
  alt_column_labels <- 
    database_columns_sub_0[[i]] %>%
      ...
      select(schema_label = object_tmp, subject_tmp)

  columns_extra <- 
    database_columns_sub_0[[i]] %>%
    distinct(subject_tmp) %>%
    left_join(alt_column_labels) %>%
    filter(!schema_label %in% column_seq_vec) %>%
    distinct() %>%
    select(subject_tmp)
