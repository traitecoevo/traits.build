...
for (thistrait in database_tables$subject_tmp) {
  ...
  anchor <- paste0(base_url, thistrait)
  writeLines(sprintf('\n\n <span id="%s"> </span>\n\n## %s ', anchor, term_to_display))

  out <- 
    create_term_table(thistrait, triples_with_labels)
...
for (thistrait in database_columns$subject_tmp) {
...
for (thistrait in database_table_entries$subject_tmp) {
...
for (thistrait in remaining$subject_tmp) {
