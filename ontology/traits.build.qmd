...
database_columns_sub <- split(database_columns, database_columns$subject_tmp)
...
for (thisterm in names(database_columns_sub)) {
  ...
  create_term_table(thisterm, database_columns_sub[[thisterm]]) %>% print_table()
}
```
...
for (thisterm in unique(database_table_entries$subject_tmp) ) {
...
for (thisterm in unique(remaining$subject_tmp)) {
