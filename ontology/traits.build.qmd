...
data_tables_sub <- split(database_tables, database_tables$subject_tmp)

for (thisterm in names(data_tables_sub)) {
  
  # creating header makes anchor using header name. Also create an anchor using identifier
  term_to_display <- data_tables_sub[[thisterm]]$Subject_labels[1]
  anchor <- paste0(base_url, thisterm)
  writeLines(sprintf('\n\n <span id="%s"> </span>\n\n### %s ', anchor, term_to_display))

  create_term_table(thisterm, data_tables_sub[[thisterm]]) %>% 
    print_table()
}
```
...
database_columns_sub_0 <- split(database_columns, database_columns$tables)

for (i in seq_along(1:nrow(database_columns_sub_0))) {

  database_tables <-
    database_tables %>% 
      distinct(subject_tmp, Subject_labels) %>%
      mutate(anchor = paste0(base_url, subject_tmp))
  
  anchor <- database_tables$anchor[match(names(database_columns_sub_0)[[i]],database_tables$Subject_labels)]
  
  writeLines(sprintf('\n\n <span id="%s"> </span>\n\n### %s ', anchor, names(database_columns_sub_0)[[i]]))
  
  database_columns_sub <- split(database_columns_sub_0[[i]], database_columns_sub_0[[i]]$subject_tmp)
  
  for (thisterm in names(database_columns_sub)) {
    
    # creating header makes anchor using header name. Also create an anchor using identifier
    term_to_display <- database_columns_sub[[thisterm]]$Subject_labels[1]
    anchor <- paste0(base_url, thisterm)
    writeLines(sprintf('\n\n <span id="%s"> </span>\n\n### %s ', anchor, term_to_display))
  
    create_term_table(thisterm, database_columns_sub[[thisterm]]) %>% 
      print_table_html()
  }
}
```
