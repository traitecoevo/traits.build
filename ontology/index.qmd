...
for (i in seq_along(1:length(database_columns_sub_0))) {
 ...
 # create vector of terms within each table from traits.build schema, to keep terms in order
  column_seq_vec <- 
    y$austraits$elements[[table_label]]$elements %>%
    names() %>%
    as.vector()
 ...
 # amend above list, to add ontology terms (as in `trait_value` instead of `value) and fiter so that it only includes terms not also in schema
  columns_extra <- 
    database_columns_sub_0[[i]] %>%
  ...
  database_columns_sub_0[[i]] <- 
    ...
    dplyr::arrange(row_n) # arranging by row number, but I think this is overwritten by `split`
