...
- Added three `plant_growth_form` substitutions to exercise the `match: word` checks added for #271 (none of their `find` values occur in `data.csv`, so they change nothing in `output/`): one with an invalid `match` value (`sometimes`, neither `word` nor `value`); and two whose `replace`/`find` chain into each other (`chain_test_a -> fern`, `fern -> herb`, both `match: word`) -- `str_replace_all()`'s named-vector form applies these sequentially, not simultaneously, so applying both together would silently turn `chain_test_a` into `herb` rather than `fern`
- `huber_value` (LASA1000)'s trait-level `collection_date` was changed from `2002/2002` to `2002-02-30` -- a calendar date that does not exist (February has 28 or 29 days) -- to exercise the `collection_date`-parses check
